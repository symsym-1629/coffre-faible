...
void STEPPER_MOTOR_init(void)
{
	for(motor_id_e m=0; m<STEPPER_MOTOR_NB; m++)
	{
		...
		//Appliquer l'état de repos sur toutes les sorties
		if(enable_gpios[m] != NULL)
...
//Cette fonction permet de faire varier la "vitesse" du moteur en modifiant la période entre deux pulses. Plus la période est courte, plus le moteur tourne vite.
//@pre newperiod doit être strictement positif, sinon la fonction ne fait rien. (pour vous en convaincre, tentez donc d'obtenir le reste d'une division par zéro...! Bon courage)
void STEPPER_MOTOR_set_period(motor_id_e id, int32_t newperiod)
{
	if(id<STEPPER_MOTOR_NB && newperiod > 0)
		pulse_period[id] = newperiod ;
}
...
static void STEPPER_MOTOR_pin_set(GPIO_TypeDef * gpio, uint32_t pin, bool b)
{
	if(gpio == NULL)
		...
		if (b)
		{
			// On bascule la broche en entrée pour que la led de l'optocoupleur ne soit pas alimentée
			gpio->MODER &= ~(0x3 << (2 * pin));
		}
		else
		{
			// On bascule la broche en sortie pour imposer le 0, ainsi la led de l'optocoupleur est alimentée
